...
import { invertNumber, PlatformSchema, PositionSchema, SizeSchema } from '../game.state'
...
const getHighestPlatform = (platforms: MapSchema<PlatformSchema>): PlatformSchema => {
    const arrayPlatforms = Array.from(platforms.values())
    if (!arrayPlatforms.length) return null
    return arrayPlatforms.reduce((max, platform) => max.body.position.y < platform.body.position.y ? max : platform);
}

export class SpawnPlatformCommand extends Command<GameRoom, SpawnPlatformPayload> {
  execute(payload: SpawnPlatformPayload) {
    ...
    const spawnPositionY = highestPlatform ? highestPlatform.body.position.y - 35 : - 35
    console.log(spawnPositionY)
